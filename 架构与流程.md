...
### 阶段 A：索引地图 + 主流程候选（project-analysis）

- 输入：后端入口（Controller/Router）、前端路由配置、认证配置
- 输出：`project-analysis-{ts}.json`
- 定位：提供端点/路由索引、链路线索（`chainHints`）、状态机线索（`stateHints`）、**主流程候选**（`mainFlowCandidates`）、页面入口（`pageEntryHints`）

### 阶段 B：主流程识别 + 用例生成

- API 侧：
  - 确认/扩展 `mainFlowCandidates` → 生成 `coverage.mainFlowInventory`
  - **四维度交叉检查**（模块生命周期 / chainHints / stateHints / crossModuleRefs）
  - 每条主流程 1～N 条用例（`trace.flowRefs` → `FLOW-*`）
  - 元信息 `mockData` 辅助外键/缺省字段
- 功能侧：
  - 确认/扩展 `mainFlowCandidates` + `pageEntryHints` → 生成 `coverage.mainFlowInventory`
  - **菜单遍历法**兜底检查
  - 每条主流程 1 条用例（`trace.flowRefs` → `FLOW-*`）
- 双侧都要求链路字段可落地：
  - `trace.dependsOn`、`trace.produces`、`trace.consumes`
...
flowchart LR
    ...
    C["mainFlowCandidates<br/>(候选预标注)"]
    I["mainFlowInventory<br/>(API + UI)"]
    T["test cases<br/>(trace.flowRefs)"]
    R["Markdown 报告"]
...
- `trace.flowRefs` → `coverage.mainFlowInventory[]`（主流程 id，格式 `FLOW-*`）
- `trace.dependsOn` → 上游用例 id
- `trace.produces` / `trace.consumes` → 变量产出/消费
...
> 已废弃：`CTR-*`、`SCN-*`、`FEAT-*`、`TC-API-WF-*`、`TC-FUNC-WF-*`
