...
flowchart LR
  ...
  EC["mainFlowInventory<br/>api-test-case-generate"]
  FI["mainFlowInventory<br/>functional-test-case-generate"]
...
```json
{
  "downstream": {
    "apiTestCaseGenerate": {
      ...
      "note": "识别 API 业务主流程（见 api-test-case-generate/主流程识别与设计.md）；读主流程 Controller/DTO；mockData 辅助外键字段"
    },
    "functionalTestCaseGenerate": {
      ...
      "note": "识别核心业务主流程（见 functional-test-case-generate/主流程识别与设计.md）；读主流程涉及页面源码；步骤用 click_menu 而非 navigate"
    }
  }
}
```
...
后续（须读源码）：
  → 接口用例：识别 API 业务主流程建 mainFlowInventory（见 api-test-case-generate/12）
  → 功能用例：识别业务主流程建 mainFlowInventory（见 functional-test-case-generate/12）
```
